...
from app.proxyman.command.command_pb2 import AddUserOperation
from app.proxyman.command.command_pb2 import AlterInboundRequest
from app.proxyman.command.command_pb2 import RemoveUserOperation
from app.proxyman.command.command_pb2_grpc import HandlerServiceStub
from app.stats.command.command_pb2 import GetAllOnlineUsersRequest
...
from proxy.vless.account_pb2 import Account as VLESSAccount
...
XRAY_CONFIG = BASE_DIR.parent.parent / "config.json"
    ...
    def init(self):
        for inbound in self._config["inbounds"]:
            if inbound["tag"] == "vless":
                inbound["settings"] = {
                    "clients": [],
                    "decryption": environ["XRAY_VLESS_DECRYPTION"],
                }
                inbound["streamSettings"] = {
                    "xhttpSettings": {"path": environ["XRAY_VLESS_PATH"]}
                }
            elif inbound["tag"] == "vmess":
                inbound["settings"] = {"clients": []}
                inbound["streamSettings"] = {
                    "xhttpSettings": {"path": environ["XRAY_VMESS_PATH"]}
                }
    ...
    def remove_user(self, email):
        ...
        for inbound in self._config["inbounds"]:
            if inbound["tag"] in ["vless", "vmess"]:
                inbound["settings"]["clients"] = list(
                    filter(
                        lambda client: client["email"] != email,
                        inbound["settings"]["clients"],
                    )
                )
